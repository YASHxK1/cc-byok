...
export async function configureCodexAppProfile({
  codexHome,
  providerId,
  providerName,
  baseUrl,
  model,
  credentialHelper,
}: CodexAppProfileInput): Promise<void> {
  ...
  await atomicWrite(
    catalogFile,
    `${JSON.stringify(createModelCatalog(model), null, 2)}\n`,
  );
}
...
export function updateCodexConfig(
  ...
  setTopLevelValue(
    lines,
    "model_provider",
    tomlString(values.providerConfigId),
  );
  setTopLevelValue(
    lines,
    "model_catalog_json",
    tomlString(values.catalogFile),
  );
  ...
  lines.push(
    ...
    'wire_api = "responses"',
    "",
    `[model_providers.${values.providerConfigId}.auth]`,
    `command = ${tomlString(process.execPath)}`,
    `args = [${tomlString(values.credentialHelper)}, ${tomlString(values.providerId)}]`,
    "timeout_ms = 5000",
...
function createModelCatalog(model: string) {
  return {
    models: [
      {
        base_instructions: "",
