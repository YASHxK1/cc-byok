...
export async function runLaunch(context, targetId, targetArgs, options = {}) {
    ...
    if (!providerId || !model) {
        throw new CliError('No active model is selected. Run "cc-byok use openrouter <model-id>" or pass --provider and --model.', "MISSING_MODEL");
    }
    const { target } = resolveTarget(config, selectedTargetId);
    const provider = requireConfiguredProvider(config, providerId);
    const compatibility = validateCompatibility(provider.definition, target.envProfile);
    if (!compatibility.compatible && !options.force) {
        throw new CliError(`${compatibility.message} Use --force to launch without an adapter check.`, "INCOMPATIBLE_TARGET");
    }
    if (compatibility.message) {
        context.output.error(`Warning: ${compatibility.message}${options.force ? " Proceeding because --force was supplied." : ""}`);
    }
    let apiKey = null;
