...
export async function runLaunch(context, claudeArgs) {
    ...
    if (!apiKey) {
        throw new CliError(`No API key is stored for ${provider.definition.displayName}. Run "cc-byok provider add ${config.activeProvider}".`, "MISSING_KEY");
    }
    ...
    context.output.log(`Launching Claude Code with ${provider.definition.displayName} (${config.activeModel})...`);
