...
import { parseProviderType, runProviderAdd, } from "./commands/provider-add.js";
...
export function createProgram(context) {
    ...
    provider
        ...
        .option("--type <type>", "provider API type", parseProviderType)
    ...
    targets.command("list").description("List launch targets").action(() => runTargetsList(context));
    targets
        .command("inspect")
        .description("Inspect a launch target")
        .argument("<target>", "target ID")
        .action((target) => runTargetsInspect(context, target));
    targets.command("add").description("Add a custom launch target").action(() => runTargetsAdd(context));
    targets
        .command("remove")
